...
*** Provisioning with preview-first `chezmoi`

During the `chezmoi` migration, install/preview, real apply, and legacy cleanup are intentionally separate steps. =ansible/local_machine.yml= remains the main provisioning entry point, and its default migration behavior stops at a safe `chezmoi` preview so you can inspect the migration boundary before the first real apply.
...
That default local-machine path currently does all of the following:

- installs =chezmoi= alongside the existing =rcm=/=rcup= workflow
- keeps =rcup= as the active deployment path for the shared dotfiles set
- saves the current =rcm=-managed baseline to =~/.local/state/dotfiles-migration/rcm-managed-files.txt=
- initializes the default empty chezmoi source directory with =chezmoi init= when needed
- runs =chezmoi apply --dry-run --verbose= only
- leaves real apply and cleanup as explicit later steps

Review the preview artifacts in =~/.local/state/dotfiles-migration/= before choosing the next step:

- =chezmoi-preview-report.md= summarizes the preview parity decision
- =chezmoi-managed-files.json= captures the managed target evidence
- =rcm-managed-files.txt= keeps the saved pre-cutover baseline

*** Apply cutover with the dedicated playbook

Use the dedicated playbook for the first real =chezmoi apply=. It reruns the preview gate, captures workstation smoke checks before and after apply, and writes the post-apply parity report plus JSON proof into =~/.local/state/dotfiles-migration/=.
...
After a successful run, inspect these cutover artifacts:

- =chezmoi-post-apply-report.md=
- =chezmoi-post-apply-report.json=
- =chezmoi-stability-before.txt=
- =chezmoi-stability-after.txt=
...
ANSIBLE_CONFIG=ansible/ansible.cfg ansible-playbook ansible/local_machine.yml --tags dotfiles,chezmoi_apply -e dotfiles_chezmoi_apply_enabled=true -e dotfiles_chezmoi_cutover_enabled=true
...
*** Cleanup legacy `rcm` links only after parity review

Cleanup never runs automatically. Only run it after the apply succeeds, the post-apply parity summary is acceptable (=clean= or =exceptions_only=), and the after-sweep workstation report ends with =- Result: passed=.

#+begin_src shell

ANSIBLE_CONFIG=ansible/ansible.cfg ansible-playbook ansible/chezmoi_apply.yml --tags chezmoi_cleanup -e dotfiles_chezmoi_cleanup_enabled=true -e dotfiles_chezmoi_cleanup_confirmation=cleanup-rcm

#+end_src

The confirmation string must be exactly =cleanup-rcm=. This keeps destructive =rcdn= cleanup separate from the first real =chezmoi apply= trust boundary.
