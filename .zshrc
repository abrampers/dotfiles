...
typeset -A __ABRAMPERS

__ABRAMPERS[ITALIC_ON]=$'\e[3m'
__ABRAMPERS[ITALIC_OFF]=$'\e[23m'
...
zstyle ':completion:*:descriptions' format %F{default}%B%{$__ABRAMPERS[ITALIC_ON]%}--- %d ---%{$__ABRAMPERS[ITALIC_OFF]%}%b%f
...
#
# Options
# See more on: http://zsh.sourceforge.net/Doc/Release/Options.html
#
...
function -report-start-time() {
  ...
  if [ $ZSH_START_TIME ]; then
    ...
    export RPROMPT="%F{cyan}%{$__ABRAMPERS[ITALIC_ON]%}${ELAPSED}%{$__ABRAMPERS[ITALIC_OFF]%}%f $RPROMPT_BASE"
...
function -record-command() {
  __ABRAMPERS[LAST_COMMAND]="$2"
}
add-zsh-hook preexec -record-command

# Update vcs_info (slow) after any command.
function -maybe-show-vcs-info() {
  local LAST="$__ABRAMPERS[LAST_COMMAND]"

  # In case user just hit enter, overwrite LAST_COMMAND, because preexec
  # won't run and it will otherwise linger.
  __ABRAMPERS[LAST_COMMAND]="<unset>"
  vcs_info
}
