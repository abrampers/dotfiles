...
#+begin_src emacs-lisp
  ...
  (menu-bar-mode t)           ; Enable the menu bar

  ;; no titlebar, but rounded
...
#+begin_src emacs-lisp
  ...
  ;; (set-face-attribute 'default nil :font "SauceCodePro Nerd Font Mono" :height abram/default-font-size :weight 'medium)
  ...
  ;; (add-to-list 'default-frame-alist '(height . abram/default-font-size))
...
#+begin_src emacs-lisp
  ...
  ;; gvm may set GOPATH correctly while leaving an older ~/bin/gopls earlier on PATH.
  ;; Prefer the active GOPATH's bin directory so executable-find resolves the same gopls.
  (defun abram/prepend-path-entry (path)
    (when path
  ...
  (defun abram/prefer-gopath-bin ()
    (when-let ((gopath (getenv "GOPATH")))
      (abram/prepend-path-entry
       (expand-file-name "bin"
                         (car (split-string gopath path-separator t))))))
...
#+begin_src emacs-lisp
  ...
  (defun abram/org-font-setup ()
    ...
    (dolist (face '((org-level-1 . 1.2)
                    ...
                    (org-level-7 . 1.05)
                    (org-level-8 . 1.05)))
      (set-face-attribute (car face) nil :font "Monego Nerd Font Fix" :weight 'regular :height (cdr face))))
...
This will setup GitHub Copilot for pair programming.
...
#+begin_src emacs-lisp
  ...
  (use-package copilot
    ...
    :init
    (setq copilot-log-level 'error)
    :bind
    (:map copilot-mode-map
          ("C-M-<next>" . copilot-next-completion)
    ...
    (:map global-map
          ("C-M-<return>" . rk/copilot-complete-or-accept))
...
#+begin_src emacs-lisp
  ...
  (use-package go-mode
    ...
    (setq lsp-go-gopls-server-path
          (expand-file-name "bin/gopls"
            (car (split-string (or (getenv "GOPATH") "~") ":"))))
    (flycheck-mode)
    :config
...
I'm following this tutorial [[https://vxlabs.com/2022/06/12/typescript-development-with-emacs-tree-sitter-and-lsp-in-2022][this tutorial]] to setup javascript development environment.

==global-tree-sitter-mode== on any buffer to activate it.
...
The following configuration will ensure that tree-sitter's dedicated ==tsx== parser will be used for tsx (typescript + react) files. By default this currently is **not** the case, as it uses the ==typescript== parser which does not understand ==tsx== extensions.

Here we create a derived mode for TSX files and configure separate mode routing: ==.ts== files use ==typescript-mode== and ==.tsx== files use ==typescriptreact-mode==. Each mode has its own tree-sitter parser mapping, ==lsp-deferred== hook, and buffer-local ==lsp-organize-imports== before-save hook. Format-on-save is handled by ==lsp-eslint-auto-fix-on-save== which runs ==eslint --fix== on save — this invokes Prettier through the ESLint plugin, respecting the project's ==.prettierrc.json== settings (including ==tabWidth==).
...
#+begin_src emacs-lisp
  ...
  (use-package typescript-mode
    ...
    (add-hook 'typescript-mode-hook
              (lambda () (setq-local lsp-eslint-auto-fix-on-save t)))
    (add-hook 'typescriptreact-mode-hook
              (lambda () (setq-local lsp-eslint-auto-fix-on-save t))))

#+end_src

[[https://github.com/orzechowskid/tsi.el/][tsi.el]] package will give us tree-sitter based indentation for TypeScript, JSON, and (S)CSS.
...
#+begin_src emacs-lisp
  (with-eval-after-load 'projectile
    (projectile-register-project-type 'lua-kong '("src/handler.lua")
                                      :project-file "src/handler.lua"
                                      :compile "make build"
                                      :src-dir "src/"
                                      :test "make spec"
                                      :test-dir "spec/"
...
this functions came from [[http://xahlee.info/emacs/emacs/emacs_url_percent_decode.html][Xah Lee]].
