...
*** Provisioning with preview-only `chezmoi`

During the `chezmoi` migration, install/preview and real apply are intentionally separate steps. =ansible/local_machine.yml= remains the main provisioning entry point, and its default migration behavior stops at a safe `chezmoi` preview so you can inspect the migration boundary before any real apply exists.
...
Real apply remains opt-in and is not part of the default =ansible/local_machine.yml= path in Phase 1.
