...
#+end_src
** TODO Ensure emacs can be ran from outside terminal
...
#+RESULTS:
| abram/go-test-keybindings | (lambda nil (setq indent-tabs-mode t) (setq tab-width 4)) | lsp-deferred | doom-modeline-env-setup-go |
...
#+RESULTS:
| (lambda nil (evil-local-set-key 'normal (kbd gd) 'lsp-find-definition) (evil-local-set-key 'normal (kbd gi) 'lsp-find-implementation) (evil-local-set-key 'normal (kbd gr) 'lsp-find-references)) | (lambda nil (evil-local-set-key 'normal (kbd gd) 'lsp-find-definition)) | rainbow-delimiters-mode |
...
#+RESULTS:
: abram/switch-to-most-recent-buffer

*** TODO run tests

**** TODO run test per file

**** TODO run test suite

**** TODO run test per function

*** TODO show treemacs
* TODO Terminal & Shell Config
